...
<div class="container-fluid">
  <?php //  @include('user.layouts.notification') ?>
    <!-- Page Heading -->
    <div class="d-sm-flex align-items-center justify-content-between mb-4">
      <h1 class="h3 mb-0 text-gray-800">{{ trans('headertrans.dashboard') }}</h1>
    </div>
    ...
    <div class="row">
      @php
          $orders=DB::table('orders')->where('user_id',auth()->user()->id)->whereIn('status', ['new','process'])->paginate(10);
      @endphp
      <!-- Order -->
      <div class="col-xl-12 col-lg-12">
        <table class="table table-bordered" id="order-dataTable" width="100%" cellspacing="0">
          <thead>
            <tr>
              <th>{{ trans('headertrans.S.N.') }}</th>
              <th>{{ trans('headertrans.OrderNo.') }}</th>
              <th>{{ trans('headertrans.Name') }}</th>
              <th>{{ trans('headertrans.Email') }}</th>
              <th>{{ trans('headertrans.Quantity') }}</th>
              <th>{{ trans('headertrans.TotalAmount') }}</th>
              <th>{{ trans('headertrans.Status') }}</th>
              <th>{{ trans('headertrans.Action') }}</th>
            </tr>
          </thead>
          <tfoot>
            <tr>
          ...
          <tbody>
            @if(count($orders)>0)
              ...
              @endforeach
              @else
                <td colspan="8" class="text-center"><h4 class="my-4">{{ trans('headertrans.NoOrders') }}</h4></td>
              @endif
          </tbody>
        </table>
